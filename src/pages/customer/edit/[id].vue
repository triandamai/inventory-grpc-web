...
import { UpdateCustomerRequest } from '@/store/customer/type';
...
const form = ref<UpdateCustomerRequest>({
  customerId: "",
  customerEmail: "",
  customerAddress: "",
  customerFullName: "",
...
async function getDetail(supplierId: string) {
  const { success, data } = await getCustomerById(supplierId)
  if (success) {
    form.value = {
...
async function onSubmit() {
  const { success } = await updateCustomer(form.value)
  if (success) {
    router.back()
  }
}
